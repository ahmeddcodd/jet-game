...
- **Radar lock and countermeasures.** Designating a bandit and holding a weapons-quality lock are separate jobs. **R** and **Tab** designate anything in radar range *in any direction* — including dead astern — and the HUD arrow then shows you which way to pull. The lock itself still has to be earned by bringing the nose onto it and holding it inside the seeker cone for ~1.15 s, so breaking hard before the tone goes solid is still a real counter. Measured end to end (designate, turn, lock): 2.2 s from 45° off, 3.4 s from dead astern. Both keys used to filter by the 25.8° cone, which meant that the moment a bandit slid off the nose — exactly when you want to call it out — neither did anything at all. Both sides carry flares.
- **You can be hunted.** A bandit chasing you asks for *your* speed plus a capped overtake, so it closes and then settles on your six instead of blowing through. Uncapped it arrived ~60 units/s hot, could not kill the overtake, and sailed past to 1500 units before spending ten seconds rejoining — which from the cockpit read as enemies randomly flying away. Capped, a merge from 600 units astern never exceeds its starting separation. Each pilot also has its own standoff and weaves in and out of it by ±34%, so bandits neither stack in one groove nor sit at a fixed range matching your speed. Over a 60-second engagement with five jets, they spend 5% of the time beyond 1200 units.
